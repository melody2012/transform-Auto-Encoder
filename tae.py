...
def capsule(images, delta):
    # 产生一个胶囊
    hidden1 = dense_layer(images, IMAGE_PIXELS,
                          NB_HIDDEN1_UNITS)   # 首先是第一个隐层，连接图片和胶囊
    ...
    # 重新生成图片（decoder）
    hidden3 = dense_layer(instantiation_params,
                          NB_INSTANTIA_PARAMS - 1, NB_HIDDEN2_UNITS)

    # 利用p重新计算输出图片
    capsule_output_images = tf.matmul(p, dense_layer(
        hidden3, NB_HIDDEN2_UNITS, IMAGE_PIXELS))
...
def __main__():
    ...
    input_deltas_placeholder = tf.placeholder(
        tf.float32, shape=(BATCH_SIZE, NB_INSTANTIA_PARAMS - 1))
    ...
    for i in progress_bar:
        ...
        delta = np.random.randint(
            low=-3, high=4, size=(BATCH_SIZE, NB_INSTANTIA_PARAMS - 1))   # 随机位移
